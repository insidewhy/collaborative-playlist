...
export class SelectedTracks extends Source {
  ...
  private selectedTracks: Observable<TrackWithIndex[]> = this.indexes.pipe(
    ...
    map(([indexes, tracks]) =>
      sortBy(mapSet(indexes, index => ({ index, track: tracks[index] })), 'index'),
    ),
    share(),
  )
  ...
  constructor(private musicQueue: MusicQueue) {
    ...
    this.reactTo(this.delete$.pipe(withLatestFrom(this.selectedTracks)), ([, selectedTracks]) => {
    ...
    this.reactTo(this.move$.pipe(withLatestFrom(this.selectedTracks)), ([offset, tracks]) => {
        ...
        ({ indexes, prevIndexes }: any, [operation, rangeStartIdx]) => {
          ...
          if (isToggle(operation)) {
            ...
            if (selectRange && rangeStartIdx !== -1) {
              ...
              if (newIndexes.has(index)) newIndexes.delete(index)
              else newIndexes.add(index)
              return { indexes: newIndexes, prevIndexes: indexes }
            }
          } else {
