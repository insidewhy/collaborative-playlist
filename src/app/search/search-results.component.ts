...
export class SearchResultsComponent extends DestructionCallbacks implements OnInit {
  ...
  public numberOfTracksFromAlbumInQueue = this.musicQueue.tracks.pipe(
    map(tracks => {
      return mapValues(
  ...
  ngOnInit() {
    ...
    this.searchResults = termsStream.pipe(
      switchMap(terms => {
        return observableCombineLatest(
          this.jsonp
            .get(
              `http://api.deezer.com/search?q=${terms}&limit=100&output=jsonp&callback=JSONP_CALLBACK`,
            )
            .pipe(
          ...
          this.jsonp
            .get(
              `http://api.deezer.com/search/album?q=${terms}&limit=100&output=jsonp&callback=JSONP_CALLBACK`,
            )
            .pipe(
              map(data => data.json().data),
              map(results => {
  ...
  private selectResult(result, play = false) {
    if (result.album) {
      ...
      if (play) this.currentTrack.play(result.id, length)
    } else {
      // TODO: move to deezer player search service
      const album = pick(result, ['title', 'id'])
      this.jsonp
        .get(
          `http://api.deezer.com/album/${result.id}/tracks?output=jsonp&callback=JSONP_CALLBACK`,
        )
        .pipe(
          map(data => data.json().data),
          map(results => {
        ...
        .subscribe(([resultTracks, tracks]) => {
          ...
          resultTracks.forEach(track => {
            this.musicQueue.insertTrack(track, newLength++)
          })
          if (play) {
